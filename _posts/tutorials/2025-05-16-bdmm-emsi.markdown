---
layout: page
title: EMSI 2025 Phylodynamics Tutorial
permalink: /tutorials/bdmm-emsi/
---
...
### Estimating the fitness of resistance mutations in *bdmm*
...
Select the ***Site Model*** panel. I simulated the sequence data under a HKY model with no rate heterogeneity across sites, so in this case we know the exact substitution model. Because there is no rate heterogeneity we can set *Gamma Category Count* to zero. For the substitution model, select HKY.
...
For the *clockRate* prior, I used an Exponential prior, with a mean of 0.01. An Exponential prior is a reasonable choice whenever parameter values need to be positive (like for rates), but we have reason to believe that higher parameter values become increasingly less probable.
...
In the MCMC panel, set the *Chain Length* to 1,000,000 iterations. This probably is not quite enough iterations but should run in the time allotted for the tutorial. Then navigate to ***File &rarr; Save*** to generate a XML input file that you can then run in BEAST. To run it, open the BEAST application and click the ***Choose file...*** button to select the XML file you just created as the *Input file*. Then click run. 

While BEAST is running, now would be a good time to download and install [Tracer][tracer] and [FigTree][figtree].

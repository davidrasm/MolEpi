...
All of this can be copied and pasted within the ```run``` element, although don't worry too much about constructing the whole XML right now because you can download a full version below.
...
The incubation reactions require a little more thought. Previously, we had the incubation rate for the $$E$$ to $$I$$ reaction set to 0.4. I propose that we follow the "80/20" rule of infectious disease epidemiology and let 80% of exposed hosts incubate into the low risk group and the other 20% incubate into the high risk group. How can we specify this in terms of the reaction rates? We can set the *IncubationLow* reaction rate four times higher than *the IncubationHigh* rate. So we will set the *IncubationLow* reaction rate to 0.32 and the *IncubationHigh* rate to 0.08 so the incubation rates still sum to an overall incubation rate of 0.4. Now, the relative probability that an exposed individual incubates into the low risk group will be $$\frac{0.32}{0.4} = 0.8$$, and for the high risk group $$\frac{0.08}{0.4} = 0.2$$.   
...
You can run the *sim_tree_seqs.py* Python script like this:
```
